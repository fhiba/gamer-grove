...
public class UserController {
    ...
    public ModelAndView postRegister(@Valid @ModelAttribute("registerForm") final RegisterUserForm registerUserForm,
    ...
    public ModelAndView manageMods(@RequestParam(value = "username", required = false) final String username,
            ...
            @ModelAttribute("removeModForm") final RemoveModForm removeModForm) throws NoLoggedUserException {
        ...
        if (Objects.nonNull(community) && !community.isEmpty() && Objects.isNull(username)) {
            ...
            try {
                ...
                LOGGER.atError().setMessage("Error getting moderators filter because username {} not found")
            ...
            try {
                ...
                LOGGER.atError().setMessage("Error getting moderators filter by username {} and community")
    ...
    public ModelAndView postAddMod(@ModelAttribute("removeModForm") final RemoveModForm removeModForm,
            @Valid @ModelAttribute("newModForm") final NewModForm newModForm, final BindingResult errors)
    ...
    public ModelAndView postRemoveMod(@ModelAttribute("newModForm") final NewModForm newModForm,
            @Valid @ModelAttribute("removeModForm") final RemoveModForm removeModForm, final BindingResult errors)
    ...
    public ModelAndView updateUser(@Valid @ModelAttribute("userPfpForm") final UserPfpForm userPfpForm,
            final BindingResult errors) throws NoLoggedUserException, UserNotFoundException {
        if (errors.hasErrors()) {
    ...
    public ModelAndView getProfileUserPosts(@RequestParam(required = false) Integer pageNumber,
            @ModelAttribute("userPfpForm") final UserPfpForm userPfpForm) throws UserNotFoundException {
    ...
    public ModelAndView getProfileLikedPosts(@RequestParam(required = false) Integer pageNumber,
            @ModelAttribute("userPfpForm") final UserPfpForm userPfpForm) throws UserNotFoundException {
    ...
    public ModelAndView getFollowedCommunities(@RequestParam(required = false) Integer pageNumber,
        ...
        List<String> selectedCategories = Arrays.asList(categories.split(",")).stream().filter(s -> !s.isEmpty())
                .toList();
    ...
    public ModelAndView getPublicProfileUserPosts(@PathVariable("id") final long id,
            @RequestParam(required = false) Integer pageNumber) throws UserNotFoundException, NoLoggedUserException {
        ...
        if (maybeLoggedUser.isEmpty()) {
            LOGGER.atError().setMessage("Not user logged found when trying to acess public profile of another user")
                    .log();
            throw new NoLoggedUserException();
        }
    ...
    public ModelAndView getFollowedCommunities(@PathVariable("id") final long id,
            @RequestParam(required = false) Integer pageNumber, @ModelAttribute("categories") final String categories)
            throws NoLoggedUserException, UserNotFoundException {
